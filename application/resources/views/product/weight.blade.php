...
<script>
    ...
    function actionData() {
        $('.edit').click(function() {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('product.weight.show') }}/" + idAppUnit,
                dataType: "json",
            ...
            iziToast.question({
                ...
                buttons: [
                    ['<button><b>OK</b></button>', function(instance, toast) {
                        ...
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('product.weight.delete') }}/" +
                                idAppUnit,
                            data: {
    ...
    $(function() {
        window.dataTableAppUnit = $("#table-product-unit").DataTable({
            ajax: "{{ route('product.weight.data-table') }}",
            processing: true,
            serverSide: true,
            order: [
                [1, 'desc']
        ...
        $('#save-product-unit').click(function() {
            ...
            $.ajax({
                type: "POST",
                url: `{{ route('product.weight.store') }}`,
                data: data,
                dataType: "json",
                success: function(response) {
        ...
        $('#edit-product-unit').click(function() {
            ...
            $.ajax({
                type: "PUT",
                url: `{{ route('product.weight.update') }}/${data.id}`,
                data: data,
                dataType: "json",
                success: function(response) {
                    $('#modal-product-unit').modal('hide')
