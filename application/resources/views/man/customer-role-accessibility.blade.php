...
<script>
    ...
    function actionData() {
        $('.edit').click(function() {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('settings.role-accessibility.show') }}/" + idAppRole,
                dataType: "json",
            ...
            iziToast.question({
                ...
                buttons: [
                    ['<button><b>OK</b></button>', function(instance, toast) {
                        ...
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('settings.role-accessibility.delete') }}/" +
                                idAppRole,
                            data: {
    ...
    $(function() {
        window.dataTableAppRole = $("#table-customer-role-accessibility").DataTable({
            ajax: "{{ route('settings.role-accessibility.data-table') }}",
            processing: true,
            serverSide: true,
            order: [
                [1, 'desc']
        ...
        $('#save-customer-role-accessibility').click(function() {
            ...
            $.ajax({
                type: "POST",
                url: `{{ route('settings.role-accessibility.store') }}`,
                data: data,
                dataType: "json",
                success: function(response) {
        ...
        $('#edit-customer-role-accessibility').click(function() {
            ...
            $.ajax({
                type: "PUT",
                url: `{{ route('settings.role-accessibility.update') }}/${data.role_id}`,
                data: data,
                dataType: "json",
                success: function(response) {
                    $('#modal-customer-role-accessibility').modal('hide')
