...
@push('css')
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700&family=Plus+Jakarta+Sans:wght@300;400;500;600&display=swap"
        rel="stylesheet">
    <style>
        :root {
            --terminal-bg: #1e1e2e;
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="#" id="form-customer-user">
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                @if (in_array(getScope(), ['global', 'manager']))
                ...
                <div class="modal-body">
                    <form action="#" id="form-create-registration-link">
                        ...
                        <div class="row">
                            @if (getScope() === 'global')
                                <div class="col mb-3">
                                    <label for="managerIdLink" class="form-label">Manager</label>
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                @if (getScope() === 'global')
                                    <label for="roleIdLink" class="form-label">User Role</label>
                                    <select class="form-control select2" name="roleIdLink" id="roleIdLink">
                                        <option value="">Select Role</option>
                                        @foreach ($customer_roles as $role)
    ...
    <script>
        ...
        function actionData() {
            ...
            $('.edit').click(function() {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ route('company.user.show') }}/" + idCustomerUser,
                    dataType: "json",
                    success: function(response) {
                ...
                iziToast.question({
                    ...
                    buttons: [
                        ['<button><b>OK</b></button>', function(instance, toast) {
                            ...
                            $.ajax({
                                type: "DELETE",
                                url: "{{ route('company.user.delete') }}/" + idCustomerUser,
                                dataType: "json",
                                success: function(response) {
                                    iziToast.success({
                                        id: 'alert-customer-user-action',
        ...
        $(function() {
            window.dataTableAppRole = $("#table-customer-user").DataTable({
                ajax: "{{ route('company.user.data-table') }}",
                processing: true,
                serverSide: true,
                order: [
                    [1, 'desc']
            ...
            $('#create-registration-link').click(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: `{{ route('company.user.registration-link') }}`,
                    data: data,
                    dataType: "json",
                    success: function(response) {
            ...
            $('#edit-customer-user').click(function() {
                ...
                $.ajax({
                    type: "PUT",
                    url: `{{ route('company.user.update') }}/${data.id}`,
                    data: data,
                    dataType: "json",
                    success: function(response) {
                        $('#modal-customer-user').modal('hide')
