...
class AppGoodUnitController extends Controller
{
    ...
    {
        try {
            if (RedisHelper::exists('app_good_units')) {
                $appGoodUnits = json_decode(RedisHelper::get('app_good_units'));
            } else {
                $appGoodUnits = AppGoodUnit::orderBy('id', 'asc')->get();
                RedisHelper::set('app_good_units', json_encode($appGoodUnits));
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $appGoodUnit = AppGoodUnit::findOrFail($id);
            $request->validate([
                'name' => 'required|min:2|max:50|unique:app_good_units,name,' . $id,
                'description' => 'required|min:6|max:100',
            ]);
